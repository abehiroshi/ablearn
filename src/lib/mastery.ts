// 習熟度エンジン（計画12）
// 概念（concept）ごとに「choice段 → input段 → 応用段」の階段を実績で昇降格する。
...
export function applyAnswer(
  ...
  if (sig.hintsUsed > 0) {
    return {
      ...m,
      streak: 0,
      dueDate: addDays(sig.today, REVIEW_INTERVALS[m.level]),
    };
  }
  // ヒントなし正解
  ...
  if (streak >= PROMOTE_STREAK && gapOk && m.level < MAX_LEVEL) {
    ...
    return {
      ...
      dueDate: addDays(sig.today, REVIEW_INTERVALS[level]),
    };
  }
  return {
    ...m,
...
/** 段位ごとの概念数（Stats の簡素な表示用） */
export function rankCounts(state: AppState): number[] {
  const counts = [0, 0, 0];
  for (const m of Object.values(state.mastery)) {
    counts[Math.min(m.level, MAX_LEVEL)]++;
  }
  return counts;
}
